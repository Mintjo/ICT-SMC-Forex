...
# Silver Bullet entry windows, in NY local time (strict 1-hour windows).
KILLZONES = {
    "london": (3, 4),
    "ny_am": (10, 11),
    "ny_pm": (14, 15),
...
def run_killzone(m5: pd.DataFrame, day: pd.Timestamp, kz_name: str, bias: str):
    """Try to find and execute one Silver Bullet trade in the given killzone on the given day."""
    ...
    sweep_rel_idx, swept_level = find_sweep(window, swing_low, swing_high, bias)
    if sweep_rel_idx is None:
        return None
    sweep_idx = m5.index.get_loc(window.index[sweep_rel_idx])

    mss_idx = find_mss(m5, sweep_idx, bias)
    if mss_idx is None:
        return None

    fvg_idx, fvg_bottom, fvg_top = find_first_fvg(m5, mss_idx, bias)
    if fvg_idx is None:
        return None

    entry_idx, entry_price = find_entry(m5, fvg_idx, bias, fvg_bottom, fvg_top)
    ...
    exit_idx, exit_price, result = simulate_exit(m5, entry_idx, bias, entry_price, sl, tp)

    pnl_price = (exit_price - entry_price) if bias == "bullish" else (entry_price - exit_price)
